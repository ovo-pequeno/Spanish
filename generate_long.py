# =========================================================
# ひとくちスペイン語｜長尺（やさしいスペイン語の短編ストーリー聞き流し・複数話）
...
Write a short, easy-to-listen original story in Spanish (level: {LEVEL}).
...
def build_video(stories):
    ...
    main_title = stories[0].get("title_ja", "ひとくちスペイン語")
    safe = main_title
    for ch in r'\/:*?"<>|':
    ...
    # エンディング
    end_txt = "Gracias por escuchar!"
    a = make_audio_tgt("Gracias por escuchar!", f"a_{idx}.mp3")
...
def upload(youtube, path, title, stories):
    ...
    description = (
        "やさしいスペイン語の短いストーリーを集めた、聞き流しスペイン語リスニング（現地語＋和訳つき）。\n"
        "作業用・睡眠用・通勤用にどうぞ。\n"
        f"Stories: {titles}\n\n#スペイン語 #スペイン語リスニング #スペイン語学習 #聞き流し #作業用BGM"
    )
    body = {
        "snippet": {
            "title": title[:100],
