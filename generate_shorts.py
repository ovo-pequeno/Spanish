# =========================================================
# ひとくちスペイン語｜Shorts（やさしいスペイン語の短編ストーリー聞き流し）
# 【完全創作】Geminiが学習者向けのやさしいスペイン語を作成。
...
def generate_story(avoid_summaries, max_retries=5):
    ...
    prompt = f"""You are a Spanish teacher creating listening material for Japanese learners.
Write a very short, easy-to-listen original story in Spanish (level: {LEVEL}).

Rules:
- Completely original. No real people, brands, or specific place names.
- Use simple, natural, everyday Spanish suitable for learners.
...
{{
  ...
  "title_tgt": "short title in Spanish",
  "sentences": [
    {{"tgt": "one sentence in Spanish.", "ja": "その日本語訳"}}
  ]
}}
sentences: 6-9 items. Each 'tgt' is one natural Spanish sentence with its 'ja' translation.{avoid_text}
"""
...
def build_video(data):
    ...
    title = data.get("youtube_title", "ひとくちスペイン語")
    safe = title
    for ch in r'\/:*?"<>|':
...
def upload(youtube, path, title):
    description = (
        "やさしいスペイン語の短いストーリーで、聞き流しスペイン語リスニング。\n"
        "現地語と和訳つき。作業用・通勤用にどうぞ。\n\n#スペイン語 #スペイン語リスニング #スペイン語学習 #聞き流し #shorts #Shorts"
    )
    body = {
        "snippet": {
            "title": (title + " #shorts")[:100],
            "description": description[:5000],
            "tags": ["スペイン語", "スペイン語リスニング", "スペイン語学習", "聞き流し", "作業用BGM", "スペイン語発音"],
            "categoryId": "27",
            "defaultLanguage": "ja",
        },
